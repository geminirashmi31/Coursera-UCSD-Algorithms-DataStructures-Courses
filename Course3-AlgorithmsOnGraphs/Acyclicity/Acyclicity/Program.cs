...
namespace Acyclicity
{
    class MainClass
    {
       ...
       private static bool DFS(Dictionary<int, IList<int>> graph, Dictionary<int, bool> visited, Dictionary<int, bool> visiting, int start)
        {
            ...
            {
                if(DFS(graph, visited, visiting, neighbors[i]) == true)
                {
                    return true;
                }
            }
